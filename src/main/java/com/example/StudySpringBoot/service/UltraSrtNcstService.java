...
public class UltraSrtNcstService {
    ...
    public List<NxNyInfo> getNxNyInfoList() {
    ...
    // 비동기 API 호출 및 데이터 저장 서비스
    @Async
    public CompletableFuture<Void> fetchAndSaveUltraSrtNcstDataAsync(NxNyInfo info) {
        ...
        return CompletableFuture.completedFuture(null); // 비동기 작업 완료 후 반환
    }

    // 전체 API 호출을 비동기적으로 수행
    public void fetchAndSaveUltraSrtNcstData() {
        List<NxNyInfo> nxNyInfoList = getNxNyInfoList();
        for (NxNyInfo info : nxNyInfoList) {
            fetchAndSaveUltraSrtNcstDataAsync(info);  // 비동기 호출
        }
    }
